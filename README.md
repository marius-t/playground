...
5. Starting the project in production mode is done via PM2 - make sure you have it installed it as a global dependency
  5.1. `npm install pm2 --global`
  5.2.  run `pm2` inside the current folder - This will make sure the process will be started in case of machine restart - 
  5.3. `pm2 save all` - is needed to save the processes to plist
